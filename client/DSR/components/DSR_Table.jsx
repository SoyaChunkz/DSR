...
const DSR_Table = ({ columns, data }) => {
  ...
  return (
    <div className="p-4">
    <button onClick={openModal} className="bg-blue-500 text-white p-2 rounded mb-4">Add Data</button>
    ...
    <div className="overflow-x-auto">
      <table className="min-w-full bg-white content-center table-auto">
        <thead>
          <tr>
            {columns.map((column) => (
              <th key={column.accessor} className="text-[13px] py-2  px-2 bg-gray-100 border border-neutral-600  mb-2">{column.header}</th>
            ))}
          </tr>
        </thead>
        <tbody className='text-[11px] '>
          {tableData.map((row, rowIndex) => (
            <tr key={rowIndex} className='content-center hover:bg-slate-100'>
              {columns.map((column) => (
                <td key={column.accessor} className="py-2 mt-2 border border-neutral-600 px-2 border-collapse">{row[column.accessor]}</td>
              ))}
            </tr>
          ))}
        </tbody>
